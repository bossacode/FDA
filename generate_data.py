...
def generate_my_data(tseq, sig=0.5, n=5000, split_size=[0.6, 0.2, 0.2], scale=True):
    """
    tseq: timepoints
    sig: std of Gaussian distribution
    n: size of data
    split_size: list containing proportion of training data, validation data, test data
    ...
    num_split_data = [int(n*p) for p in split_size]
    leftover = n - sum(num_split_data)
    if leftover != 0:
        for i in range(leftover):
            num_split_data[i] += 1
    original_train, original_val, original_test = original_data.split(num_split_data)
    noise_train, noise_val, noise_test = noise_data.split(num_split_data)

    # save data
    if not os.path.exists("data/mydata/"):
        os.makedirs("data/mydata/train/")
        os.makedirs("data/mydata/val/")
        os.makedirs("data/mydata/test/")
    torch.save(original_train, "data/mydata/train/original_train.pt")
    torch.save(noise_train, "data/mydata/train/noise_train.pt")
    torch.save(original_val, "data/mydata/val/original_val.pt")
    torch.save(noise_val, "data/mydata/val/noise_val.pt")
    torch.save(original_test, "data/mydata/test/original_test.pt")
    torch.save(noise_test, "data/mydata/test/noise_test.pt")
